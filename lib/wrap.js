...
Wrap.prototype.require = function (mfile, opts) {
    ...
    nub(required.strings).forEach(function (req) {
        ...
        if (opts.target && /^[.\/]/.test(req)) {
            ...
            // make a relative path from this by removing the local prefix
            var cwd = process.cwd();
            var cwdRegExp = new RegExp("^" + cwd);
            var reqFilenameRelative = cwdRegExp.test(reqFilename) ?
                reqFilename.slice(cwd.length) : reqFilename;

            // get rid of drive letter on Windows; replace it with '/'
            var reqFilenameWithoutDriveLetter = /^[A-Z]:\\/.test(reqFilenameRelative) ?
                '/' + reqFilename.substring(3) : reqFilenameRelative;
